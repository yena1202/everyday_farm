...
//#############################################
		...
		ImageIcon[] image = new ImageIcon[5];//우리를 이미지 배열로 선언,생성
		...
		lblCage = new JLabel[5];//우리이미지를 넣은 JLabel 배열
		
		for(i=0;i<5;i++){//for문을 통해 JLabel배열에 이미지배열을 추가한다.
...
//#############################################  
		...
		private class BtnListener implements ActionListener {
			...
			{
				//btnStartSheep을 누르면 기존의 FarmerPanel 에서 DayPanel로 화면 전환이 이루어짐
				FarmPanel.setVisible(false);
				DayPanel.setVisible(true);
			    myThreadStart();
			    type = 1;//
			    temp = 1;//SheepType 을 저장해 놓는 temp 
				color(1);
